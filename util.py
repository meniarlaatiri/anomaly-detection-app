...
class TS_Featurizing :
  ...
  def load_and_save_data(self):
     ...
     df = pd.read_csv(self.src, parse_dates=[self.x_src], date_parser=dateparse,usecols=[self.x_src,self.y_src], index_col=0, sep='[,;]')
    #  df = pd.read_csv(self.src, parse_dates=[self.x_src], date_parser=dateparse,usecols=[self.x_src,self.y_src], index_col=0, sep=';')
     df= df.dropna()
     return df
